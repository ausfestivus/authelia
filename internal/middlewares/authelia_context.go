...
func (ctx *AutheliaCtx) GetSession() (userSession session.UserSession, err error) {
	...
	// The provider validates that the session is bound to this cookie domain, so a failure here other than the backend
	// failing means the session is either unreadable or was moved between domains, both of which are handled by falling
	// back to a new session. A backend failure is returned so the caller doesn't treat the user as anonymous.
	if current, err = provider.Get(ctx); err != nil {
		if errors.Is(err, session.ErrRepositoryGet) {
			return userSession, err
		}
